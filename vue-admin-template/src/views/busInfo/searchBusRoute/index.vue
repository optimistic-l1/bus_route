<template>
  <div class="app-container" :style="{ height: windowHeight - 50 + 'px' }">
    ...
    <div class="input-info">
      <p><el-tag>公交线路</el-tag> {{busName}}</p>
      <p><el-tag>单程最高票价</el-tag> {{busPrice}}</p>
      <el-collapse v-model="activeNames" >
        <el-collapse-item title="详情路线" type="text" name="1">
          <p v-for="(item,i) in busPath">{{i}}.{{item}}</p>
        </el-collapse-item>
      </el-collapse>
    </div>
    ...
    <div class="input-card" style="width: 18rem">
      ...
      <el-button round type="mini" style="width: 100%" @click="lineSearch"
        >查询</el-button
      >
    </div>
  </div>
  ...
  methods: {
    ...
    lineSearch() {
      ...
      linesearch.search(that.busLineName, function (status, result) {
        ...
        //公交线路基本信息
        let busName = result.lineInfo[0].name;
        let busPrice = result.lineInfo[0].total_price;
        //公交站点详情
        let busPath = result.lineInfo[0].via_stops;
        that.busName = busName;
        that.busPrice = busPrice;
        if (busPath.length == 0) {
        } else {
          for (let i = 0; i < busPath.length; i++) {
            that.busPath.push(busPath[i].name);
          }
        }
